...
export async function POST(
  ...
  const taskId = `adv_retest_${id}_${Date.now()}`;
  registerTask(taskId, async (shouldAbort) => {
    const result = await runAdvancedRetest(id, mode, shouldAbort);
    if (!result) {
      throw new Error("未找到高级测试问题，无法执行复测");
    }
    return { result };
  });
